...
do
    ...
    if [ "${USE_TWOPLAY}" -eq 1 ] && [ "${ITER}" -ge 1 ]; then
        sleep 3
        cp "${workdir}/selfimprove/${model_name}-all-sftdata_${ITER}_newcritic.json" "${workdir}/LLaMA-Factory/data/selfimprove_critic.json"
        sleep 3

        base_model_name="meta-llama/Meta-Llama-3-8B-Instruct"

        cd ${workdir}/LLaMA-Factory
        echo "Critic SFT ${ITER} started."
        llamafactory-cli train \
            --stage sft \
            --do_train True \
            --model_name_or_path ${base_model_name} \
            ...
            --flash_attn fa2 \
            --dataset_dir data \
            --dataset gsm8k_train_chat5,math_train_chat,selfimprove_critic \
            --cutoff_len 4096 \
            --learning_rate 5e-06 \
            ...
            --packing False \
            --report_to none \
            --output_dir ${workdir}/models/${model_name}-all-critic${EXP_NUM}_${USE_CRITIC}_${ITER} \
            --bf16 True \
            --plot_loss True \
            --ddp_timeout 180000000 \
            --include_num_input_tokens_seen True \
        ...
        echo "Critic SFT ${ITER} finished."
        sleep 5
        critic_model_name="${workdir}/models/${model_name}-all-critic${EXP_NUM}_${USE_CRITIC}_${ITER}"
    fi
done
cd ${workdir}/selfimprove
